...
func (p *PrometheusStore) Series(r *storepb.SeriesRequest, s storepb.Store_SeriesServer) error {
	...
	if strings.HasPrefix(contentType, "application/x-protobuf;") {
		return p.handleSampledPromethuesResponse(s, httpResp, queryPrometheusSpan, externalLabels)
	}
...
func (p *PrometheusStore) handleSampledPromethuesResponse(s storepb.Store_SeriesServer, httpResp *http.Response, querySpan opentracing.Span, externalLabels labels.Labels) error {
	ctx := s.Context()
